...
| `cron-cleanup.test.ts` | Daily housekeeping (ordered) | `GET /api/cron/daily-cleanup` | Bearer auth required; ordered: expired invites → sessions >90d → orphaned tracked-users → empty orgs (with `activeOrganizationId` nullify FIRST) |
| `sessions-detail.test.ts` | Session + slices read shape | `GET /api/sessions/[id]`, `GET .../slices/[n]/events` | Cross-org access → 404 (NOT 403 — avoids existence leak) |
