...
| **Pages**    | Number of slices in the session (one per SPA navigation / idle gap)                                                                  |
...
the server resolves the next slice with `cursor: { id }, skip: 1`.
...
slices, and its events are removed from the database. No soft-delete.
The session is gone.
...
- [Resources → Data model](/documentation/resources/data-model) — the `Session` / `Slice` / `Event` schema underneath.
