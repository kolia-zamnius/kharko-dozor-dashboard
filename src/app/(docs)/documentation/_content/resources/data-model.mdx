...
- **Business side** — `Organization`, `Membership`, `Invite`,
  `Project`, `TrackedUser`, `Session`, `Slice`, `Event` (eight
...
| `Session` | `externalId`, `projectId` (FK → Project, Cascade), `trackedUserId` (FK → TrackedUser, SetNull), `url`, `userAgent`, `duration`, `eventCount`, `startedAt`, `endedAt` | Owns: Slice (Cascade), Event (Cascade) | One browser session as captured by the SDK |
| `Slice` | `index`, `reason` (`init \| idle \| navigation`), `url`, `pathname`, `duration`, `startedAt`, (`sessionId`, `index`) unique | Belongs to Session, owns Events (SetNull on delete) | Replayable segment within a session |
| `Event` | `type`, `timestamp`, `data` (Json), `sessionId` (FK → Session, Cascade), `sliceId` (FK → Slice, SetNull), `sliceIndex` | Belongs to Session + Slice | Raw rrweb event |
...
| Slice events (one slice's events) | `sliceId` on `Event` |
| Session events fallback (legacy pre-slice) | `sessionId` on `Event` |
...
| `Slice` | 250 | ~3× session count (init + idle + navigation) |
| `Event` | 100 + data JSON (avg 200 bytes) | rrweb-volume — **dominant table by 10–100×** |
