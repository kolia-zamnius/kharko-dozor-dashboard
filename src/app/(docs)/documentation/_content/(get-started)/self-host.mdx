...
2. **Old sessions** — `Session` rows older than `SESSION_RETENTION_DAYS` (default 90). Cascades to `EventBatch` + `Marker` rows, which is the bulk of database volume — gzipped event blobs for one busy session can be tens of MB.
